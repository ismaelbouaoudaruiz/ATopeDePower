...
function decrementDailyStats() {
    ...
    for (const activity in dailyDecrementEffects) {
        const daysInactive = Object.keys(activitiesDoneToday).filter(date => {
            const dayDifference = (new Date(today) - new Date(date)) / (1000 * 60 * 60 * 24);
            return dayDifference >= 2 && !activitiesDoneToday[date].has(activity);
        }).length;
        
        if (daysInactive > 0) {
            const effect = dailyDecrementEffects[activity];
            for (const stat in effect) {
                stats[stat] += effect[stat] * daysInactive; // Decrement based on days inactive
